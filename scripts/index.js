...
window.onload = function () {
  ...
  sortEmployeesByName();
  let employeeNameList = document.querySelector("#employeeNameList");
  let personInformationTB = document.querySelector("#personInformationTB");

  function listEmployeeName() {
    for (const employee of employees) {
      let nameOption = new Option("option");
      nameOption.value = employee.id;
      nameOption.innerText = employee.name;

      employeeNameList.appendChild(nameOption);
    }
  }


  function loadEmployeeDetails() {
    personInformationTB.innerHTML = "";

    for (const employee of employees) {
      if (employee.id == employeeNameList.value) {
        let row = personInformationTB.insertRow();
        ...
        let jobTitleCell = row.insertCell(2);
        jobTitleCell.innerText = employee.jobTitle;

        let yearsAtCompanyCell = row.insertCell(3);
        yearsAtCompanyCell.innerText = employee.yearsAtCompany;

        let emailCell = row.insertCell(4);
        emailCell.innerText = employee.email;

        let wfhAddressCell = row.insertCell(5);
        wfhAddressCell.innerText = employee.wfhAddress;

        let skillSetCell = row.insertCell(6);
        skillSetCell.innerText = employee.skillSet;

        let numberOfprojectCell = row.insertCell(7);
        numberOfprojectCell.innerText = employee.projectsAssignedTo.length;
      }
    }
  }

  employeeNameList.onchange = loadEmployeeDetails;
